YUI.add('api-list', Y => {
    var Lang   = Y.Lang;
    var YArray = Y.Array;
    var APIList = Y.namespace('APIList');
    var classesNode    = Y.one('#api-classes');
    var inputNode      = Y.one('#api-filter');
    var modulesNode    = Y.one('#api-modules');
    var tabviewNode    = Y.one('#api-tabview');
    var tabs = APIList.tabs = {};

    var filter = APIList.filter = new Y.APIFilter({
        inputNode,
        maxResults: 1000,
    ...
    var search = APIList.search = new Y.APISearch({
        inputNode,
        maxResults: 100,
    ...
    var tabview = APIList.tabview = new Y.TabView({
        srcNode  : tabviewNode,
    ...
    var focusManager = APIList.focusManager = tabviewNode.plug(Y.Plugin.NodeFocusManager, {
        circular   : true,
    ...
    tabview.get('panelNode').all('a').each(link => {
    ...
    function onFilterResults(e) {
        ...
        var resultNode   = getFilterResultNode();
        var typePlural   = filter.get('queryType');
        var typeSingular = typePlural === 'classes' ? 'class' : 'module';

        if (e.results.length) {
            YArray.each(e.results, result => {
